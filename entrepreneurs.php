<?php 
$_SESSION["lan"] = "English";
?>
<?php include 'inc/header.php'; ?>
<body>
	<section id="entrepreneurs" class="entrepreneurs_section bg-white pt-100">
		<div class="container">
			<div class="row no-gutters">
					<div class="col-12 strike_heading_2 mt-3">
	                    <h2 class="stroke_text text-uppercase text-center text-lg-left text-sm-left text-xs-left mb-10 mb-lg-0 wow fadeInLeft">
	                    	Searchers
	                    </h2>
	                </div>
	                <div class="col-lg-7">
	                	<div class="position-relative z-index-1">
							<div class="headline">
							
							<h4 class="mb-5 mt-3 subtitle wow fadeInUp">
								Only exceptional operators, who are ready to embark on a unique journey, are chosen for our Operator-Led Search Fund Program. Learn more about our current cohort of NCA Searchers below.
							</h4>
							</div>
						</div>
	                </div>	
	                <div class="col-lg-5  d-flex justify-content-end align-items-end">
	                	
	                </div>	
				</div>
			<!-- <div class="pb-100">
				<div class="stroke_wrap">
					<h2 class="stroke_text talents_stroke_text talents_strok_cls_2 text-uppercase mb-20 wow fadeInLeft pt-70">
						Searching Entrepreneurs
					</h2>
				</div>
				<div class="row no-gutters d-flex align-items-center">
					<div class="col-12 col-lg-12">
						
						<div class="col-12">
							<p class="enterpreneurs_top_text mt-3 font__family-open-sans text-content wow fadeInUp font__size-24">
								Only exceptional talents, who are ready to embark on an exceptional journey, are chosen for our ETA program. Learn more about our current cohort of NCA Searching Entrepreneurs below.
							</p>	
						</div>
					</div>
					<div class="col-12 col-lg-8">
						<div class="responsive_xs_pad px-5">
							<div class="brk-ic-left-slide image_caption mt-lg-0 mt-5" data-brk-library="component__image_caption_css">
								<img src="img/stay_tuned.png" alt="alt" class="image_video w-100">
                            </div>
                    <div class="play_button left_btn center_btn brk-hosted-video rendered brk-library-rendered talents_video_btn" data-brk-library="component__media_embeds,fancybox">
                    	<div class="brk-hosted-video__body">
                    		<a class="d-flex align-items-center fancybox brk-hosted-video__btn" href="https://youtu.be/FICy-KXv4DM" data-brk-library="component__button">
                    			<svg width="138" height="134" viewBox="0 0 138 134" fill="none" xmlns="http://www.w3.org/2000/svg">
                    				<path d="M68.9999 0C55.3488 0 42.0042 3.93069 30.6537 11.295C19.3032 18.6594 10.4565 29.1266 5.23248 41.373C0.00841832 53.6194 -1.35843 67.0951 1.30477 80.0958C3.96798 93.0966 10.5416 105.039 20.1944 114.412C29.8472 123.785 42.1457 130.168 55.5345 132.754C68.9233 135.34 82.8012 134.012 95.4132 128.94C108.025 123.867 118.805 115.277 126.389 104.255C133.973 93.234 138.021 80.2762 138.021 67.0207C138.021 49.2457 130.749 32.1987 117.805 19.6299C104.861 7.06109 87.3055 0 68.9999 0ZM79.3673 79.6925L56.7041 92.3919V41.5874L79.3531 54.2799L102.002 66.9793L79.3673 79.6925Z" fill="#D6762C"/>
                    			</svg></a>
                    		</div>
                    	</div>
                    </div>
                </div>
            </div>
        </div> -->
    </div>
</section>
<section class="our_team bg-white">
	<div class="container">	
		<div class="row no-gutters new_row pb-130">
			
<!-- 
		<div class="  col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-vanessa-monestel.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
                        	    Vanessa Monestel 
                        	</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/searcher-vanessa.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>		
					    </div>	
                    </div>	
				</div> -->
				 
		 
				
 
 

			
			 


				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-olivier guillebert.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Olivier Guillebert</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/olivier-guillebert.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-alejandro-salcedo.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Alejandro Salcedo</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/alejandro-salcedo.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				
				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-aron-szabo.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Aron Szabo</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/aron-szabo.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				
				<div class="col-lg-3 member_card mt-10 mt-lg-0 pl-5 pr-5">
                    	<div class="bg-primary question-box question-box-height box-height-entrepre">
							<a href="talents.php" class="d-flex align-items-center py-5 px-4 text-white h-100">
							<h2 class="text-white mb-1 font__family-efb text-uppercase">Interested in joining us?</h2>
							<div class="text-right arrow_link">
								<svg id="SMALL_ARROW" data-name="SMALL ARROW" xmlns="http://www.w3.org/2000/svg" width="34.975" height="34.976" viewBox="0 0 34.975 34.976">
								  <path id="Path_10" data-name="Path 10" d="M17.488,0l-3.18,3.18L26.345,15.217H0v4.542H26.345L14.308,31.8l3.18,3.18L34.975,17.488Z" fill="#fff"/>
								</svg>

							</div>
							</a>
						</div>	
				</div>

				

				 
	

			  <div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-erika-obrien.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Erika O'Brien</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/erika-obrien.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-ben-boenk.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Ben Boenk</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/ben-boenk.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>


				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-marcin-chruciel.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Marcin Chruściel</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/marcin-chrusciel.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>


				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-jordan-frank.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Jordan Frank</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/jordan-frank.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>
				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-bill-balliete.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Bill Balliette</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/bill-balliette.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>
				 
				<!-- <div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-jim-kelly.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Jim Kelly</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/jim-kelly.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div> -->
				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-simon-farmer.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Simon Farmer</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/simon-farmer.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<!-- <div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-martine-legault.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Martine Legault</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/martine-legault.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div> -->

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-eyal-keplan.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Eyal Kaplan</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/eyal-kaplan.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>


				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-paulo-riberinho.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Paulo Ribeirinho</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/paulo-ribeirinho.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>



				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-max-koutny.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Max Koutny</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/max-koutny.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>
				





				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/search-brooks-bergreen.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Brooks Bergreen </h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/brooks-bergreen.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>



				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-carlos-laconi.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Carlos Laconi </h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/carlos-laconi.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>


				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-abed-shaheen.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Abed Shaheen </h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/abed-shaheen.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<!-- <div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-krishan-arora.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Krishan Arora</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/krishan-arora.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div> -->

				<!-- <div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-george-bassily.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">George Bassily</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/george-bassily.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div> -->

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-padraig-lawlor.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Padraig Lawlor</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/padraig-lawlor.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-justin-mack.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Justin Mack</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/justin-mack.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/david_kirubi_about.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">David Kirubi</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/david-kirubi.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				
				<!-- <div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-jen-liang.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Jen Liang</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/jen-liang.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div> -->

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-morgan-eriksen.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Morgan Ericksen</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/morgan-ericksen.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-mark-lopez.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Mark Lopez</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/mark-lopez.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-robert-vero.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Robert Vero</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/robert-vero.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>


				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-bill-reese.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Bill Rees</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/bill-rees.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-nell-smircina.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Nell Smircina</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/nell-smircina.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-steward-gill.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Stewart Gill</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://www.novastone-ca.com/stewart-gill.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-rouven-behge.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Rouven Behge</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/rouven-behge.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-carlo-salizzoni.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Carlo Salizzoni</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/carlo-salizzoni.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-jeffry-revoy.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Jeffrey Revoy</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/jeffrey-revoy.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/searcher-ajay-lakhwani.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Ajay Lakhwani</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/ajay-lakhwani.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/about-teresa-rupp.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Teresa Rupp</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/teresa-rupp.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/about-francis-cohen.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Francis Cohen</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/francis-cohen.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/about-michal-mirowski.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Michal Mirowski</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/michal-mirowski.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/about-vivan-pinto.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Vivan Pinto</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/vivan-pinto.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/about-ralf-aigner2.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Ralf Aigner</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/ralf-aigner.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/matthieu-mary-about.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Matthieu Mary</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/matthieu-mary.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/reed-davis-about.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Reed Davis</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/reed-davis.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/josh-about.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Josh Seegopaul</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/josh-seegopaul.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/about-michael-forbis.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Michael Forbis</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/michael-forbis.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				
				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/about-nicola-taglioli.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Nicola Taglioli</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/nicola-taglioli.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/about-zaretti.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Christophe Zaretti</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/christophe-zaretti.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>

				<div class="col-lg-3 member_card mt-10 mb-10 mt-lg-0 pl-5 pr-5">
                    <div class="img_section w-100">
                    	<img class="w-100" src="img/about-karim.png">
                    </div>	
                    <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                    	<div class="text-white">
                        	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">Karim Kuperhause</h5>
		        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
							<div class="d-flex align-items-center justify-content-center">
									<a href="https://novastone-ca.com/karim-kuperhause.php" target="_self" class="font__size-13 font__family-open-sans btn btn-prime learn_more_btn bg-transparent2 text-capitalize border-radius-25 btn-no-shadow btn_lang px-3 font__weight-bold" data-brk-library="component__button">Learn more</a>
								</div>	
		        			</p>	
					</div>	
                    </div>	
				</div>






				











				 
				 
				
			</div>								
	    </div>				
	</div>	
</section>

<section class="pt-5 pb-5">
	<div class="container">	
		<div class="row">
			<div class="col-12">
				<p class="font__size-24">
					Once Searchers complete a successful acquisition they become NCA CEOs.
				</p>	
			</div>	
		</div>
	</div>		
</section>

<section id="ceos" class="entrepreneurs_section bg-white pt-100 pb-100">
		<div class="container">
			<div class="pb-100">
				<div class="stroke_wrap pt-3">
					<h2 class="stroke_text talents_stroke_text talents_strok_cls_2 text-uppercase mb-20 wow fadeInLeft">
						NCA CEOs
					</h2>
				</div>
				<div class="row no-gutters d-flex align-items-center">
					<div class="col-12 col-lg-4">
						
						<div class="col-12">
							<p class="font__family-open-sans text-content wow fadeInUp font__size-20 pl-50 pl-sm-0">
								Following a successful acquisition, our Searchers become CEOs and step into a leadership role at their new companies. NCA supports CEOs in value creation and serves on the boards alongside other co-investors. Learn more about our CEOs and their companies below.
							</p>	
						</div>
					</div>
					<div class="col-12 col-lg-8">
						<div class="responsive_xs_pad px-5">
							<div class="brk-ic-left-slide image_caption mt-lg-0 mt-5" data-brk-library="component__image_caption_css">
								<img src="img/srsdfsdf.png" alt="alt" class="image_video">
                            </div>
                    <div class="play_button left_btn center_btn brk-hosted-video rendered brk-library-rendered talents_video_btn" data-brk-library="component__media_embeds,fancybox">
                    	<div class="brk-hosted-video__body">
                    		<a class="d-flex align-items-center fancybox brk-hosted-video__btn" href="https://youtu.be/iqDKHmlWwWI" data-brk-library="component__button">
                    			<svg width="138" height="134" viewBox="0 0 138 134" fill="none" xmlns="http://www.w3.org/2000/svg">
                    				<path d="M68.9999 0C55.3488 0 42.0042 3.93069 30.6537 11.295C19.3032 18.6594 10.4565 29.1266 5.23248 41.373C0.00841832 53.6194 -1.35843 67.0951 1.30477 80.0958C3.96798 93.0966 10.5416 105.039 20.1944 114.412C29.8472 123.785 42.1457 130.168 55.5345 132.754C68.9233 135.34 82.8012 134.012 95.4132 128.94C108.025 123.867 118.805 115.277 126.389 104.255C133.973 93.234 138.021 80.2762 138.021 67.0207C138.021 49.2457 130.749 32.1987 117.805 19.6299C104.861 7.06109 87.3055 0 68.9999 0ZM79.3673 79.6925L56.7041 92.3919V41.5874L79.3531 54.2799L102.002 66.9793L79.3673 79.6925Z" fill="#D6762C"/>
                    			</svg></a>
                    		</div>
                    	</div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<section class="logo_img_section bg-white pb-100">
	<div class="container">
		<div class="row">
			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="docs/NCA-CELEBRATES-FIRST-ACQUISITION.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/sebasstien-ceo.png">
	                	</div>   
	                </a>
	                <a href="https://iargroup.com/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center">
	                	<div class="logo_div">
	                	    <img class="w-50" src="img/IAR-Dark-Desktop.png">
	                	</div>    
	                </a>	
	            </div>    	
                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
                    		Sébastien Perroud  
                    	</h5>
                        <p class="text-center font__size-16 font__family-open-sans line__height-34">
	        				CEO IARGroup
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/sebastienperroud/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>
            

			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="docs/NCA-CELEBRATES-THIRD-ACQUISITION.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/mike-ceo.png">
	                	</div>   
	                </a>
	                <a href="https://glgbv.com/en/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/green-logistics.png">
	                	</div>    
	                </a>	
	            </div>    	
                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
                    		Mike Hindi 
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
	        				CEO GreenLogistics
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/mike-hindi" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>


			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="docs/NCA-CELEBRATES-ITS-NEXT-ACQUISITION-IN-POLAND.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/val-ceo.png">
	                	</div>   
	                </a>
	                <a href="https://www.formeds.com.pl/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/formeds.png">
	                	</div>    
	                </a>	
	            </div>    	
                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						Waldemar Pilch
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
	        				CEO ForMeds
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/valpilch/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>


			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="docs/SUCCESSFUL-UK-ACQUISITION-FOR-NCA-ENTREPRENEUR-WILL-HUNNAM.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/will-ceo.png">
	                	</div>   
	                </a>
	                <a href="https://www.forza-doors.com/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/forza.png">
	                	</div>    
	                </a>	
	            </div>    	
                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						Will Hunnam
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
	        				CEO Forza Doors
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/will-hunnam-11877b28/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>



			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="docs/SUCCESSFUL-ACQUISITION-FOR-NCA-ENTREPRENEUR-GEORGE-BERCZELY-FINAL-May-2023.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/george-ceo.png">
	                	</div>   
	                </a>
	                <a href="https://seyses.com/es/en/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/seyses.png">
	                	</div>    
	                </a>	
	            </div>    	
                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						George Berczely
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						CEO SEYSES
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/george-berczely/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>


			<!-- <div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="docs/SUCCESSFUL-ACQUISITION-FOR-NCA-ENTREPRENEUR-DAVID-MILLER.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/david-miller-ceo.png">
	                	</div>   
	                </a>
	                <a href="https://www.chapter-1.net/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/chapter1.png">
	                	</div>    
	                </a>	
	            </div>    	
                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						David Miller
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						CEO Chapter One Sportswear Inc.
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/davidmillermontreal/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div> -->


			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="docs/SUCCESSFUL-ACQUISITION-FOR-NCA-ENTREPRENEUR-NICK-AKERS.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/nick-akers-ceo.png">
	                	</div>   
	                </a>
	                <a href="https://inzotechnologies.com/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/portfolio/inzo.png">
	                	</div>    
	                </a>	
	            </div>    	
                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						Nick Akers
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						CEO Inzo Technologies
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/akersnick/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>




			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="docs/SUCCESSFUL-ACQUISITION-FOR-NCA-ENTREPRENEUR-NATE-TAYLOR.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/nathan-taylor-ceo.png">
	                	</div>   
	                </a>
	                <a href="https://waterrunner.com/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/water-runner-logo.png">
	                	</div>    
	                </a>	
	            </div>    	
                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						Nate Taylor
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						CEO Water Runner
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/nathanptaylor/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>


			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="#" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/daniel-about.png">
	                	</div>   
	                </a>
	                <a href="https://fooseng.com/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/foo-seng.png">
	                	</div>    
	                </a>	
	            </div>   

                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						Daniel Coutinho
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						CEO Foo Seng
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/danielcoutinho/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>



			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="https://novastone-ca.com/docs/SUCCESSFUL_ACQUISITION_FOR_NCA_ENTREPRENEUR_VIKTOR_DIMITROV.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/viktor-about.png">
	                	</div>   
	                </a>
	                <a href="https://www.med-systeme.com/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/Med-Systeme-FF.jpg">
	                	</div>    
	                </a>	
	            </div>  

                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						 Viktor Dimitrov
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						 CEO Med-Systeme GmbH 
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/viktordimitrov/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
				    </div>	
                </div>	
			</div>





			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="https://novastone-ca.com/docs/SUCCESSFUL-ACQUISITION-FOR-NCA-ENTREPRENEUR-FRYDERYK-RDU%C5%81TOWSKI-FINAL.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/fryderyk-about.png">
	                	</div>   
	                </a>
	                <a href="https://adexcp.com/en/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/adex.jpg">
	                	</div>    
	                </a>	
	            </div>   

                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						  Fryderyk Rdułtowski
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						CEO adéx
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/fryderyk-g-rdu%C5%82towski-74bb163/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>




			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="https://novastone-ca.com/docs/NCA-Announces-Another-Successful-USA-Acquisition.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/ryan-about.png">
	                	</div>   
	                </a>
	                <a href="https://www.stocktontruss.com/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/stockton-carson-truss.jpg">
	                	</div>    
	                </a>	
	            </div>   

                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						  Ryan Wright
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						  CEO Stockton Truss & Carson Truss LLC
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/ryanwright3/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>



			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="https://www.novastone-ca.com/docs/SUCCESSFUL-ACQUISITION-FOR-NCA-ENTREPRENEUR-ROBERT-WAGNER.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/robert-wagner.png">
	                	</div>   
	                </a>
	                <a href="http://www.sor4life.com/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/stewards-of-recovery-logo.png">
	                	</div>    
	                </a>	
	            </div>   

                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						  Robert Wagner
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						  CEO Stewards of Recovery <br>
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/robert-wagner-healthcare-investor/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>





			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="https://novastone-ca.com/docs/SUCCESSFUL_ACQUISITION_FOR_NCA_ENTREPRENEUR_NICOLETTA_NUTRITO.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/nicoletta_nutrito.png">
	                	</div>   
	                </a>
	                <a href="https://www.mecavit.it/it/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/mecavit.png">
	                	</div>    
	                </a>	
	            </div>   

                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						  Nicoletta Nutrito
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						  CEO Mecavit S.R.L <br>
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/nicoletta-nutrito-gm-bu-services/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>



			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="https://novastone-ca.com/docs/SUCCESSFUL-ACQUISITION-FOR-NCA-ENTREPRENEUR-DEAN-CURRAN-FINAL.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/dean2-about.png">
	                	</div>   
	                </a>
	                <a href="https://www.garnerosborne.co.uk/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/Garner-Osborne.svg">
	                	</div>    
	                </a>	
	            </div>   

                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						  Dean Curran
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						  CEO Garner Osborne<br>
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/dean-curran-7925b94/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>



			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/marc-decoste-ceo.png">
	                	</div>   
	                </a>
	                <a href="http://extendedpowerinc.com/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/Extended-Power-Logo.png">
	                	</div>    
	                </a>	
	            </div>   

                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						 Marc Decoste
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						  CEO Extended Power Inc.<br>
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/marc-decoste-09a29211/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>



			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="https://novastone-ca.com/docs/SUCCESSFUL_ACQUISITION_FOR_NCA_ENTREPRENEUR_LISA_PIERCEY.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/Lisa_CEO.png">
	                	</div>   
	                </a>
	                <a href="https://keystone-pharmacy.com/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/keystone-pharmacy.png">
	                	</div>    
	                </a>	
	            </div>   

                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						  Lisa Piercey
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						  CEO Keystone Specialty Pharmacy<br>
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/lisapierceymd/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>



			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="https://www.novastone-ca.com/docs/SUCCESSFUL_ACQUISITION_FOR_NCA_ENTREPRENEUR_PAOLO_CIARLARIELLO.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/paolo-ciarlariello-about.png">
	                	</div>   
	                </a>
	                <a href="https://www.marcona3.com/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/Showroom-Marcona3.png">
	                	</div>    
	                </a>	
	            </div>   

                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						  Paolo Ciarlariello
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						  CEO Showroom Marcona 3<br>
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/paolo-ciarlariello-%E6%9F%A5%E6%98%8E%E7%9D%BF-70b1a2/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	
			</div>


			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="https://www.novastone-ca.com/docs/SUCCESSFUL%20RECAPITALIZATION%20FOR%20NCA%20ENTREPRENEUR%20ADAM%20SALMEN_OCT%202%202024.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/adam-salmen-ceo.png">
	                	</div>   
	                </a>
	                <a href="https://umecc.com/about.html" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/logo-umecc.png">
	                	</div>    
	                </a>	
	            </div>   

                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						  Adam Salmen
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						CEO UMECC<br>
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/adam-salmen-131a675/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	

			</div>

			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="docs/NEW_EQUITY_PARTNERSHIP_FOR_NCA_ENTREPRENEUR_JIM_KELLY.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/searcher-jim-kelly.png">
	                	</div>   
	                </a>
	                <a href="https://gdipackaging.ca/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/GDILOGO.jpg">
	                	</div>    
	                </a>	
	            </div>   

                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						  Jim Kelly
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						CEO GDI Packaging Solutions<br>
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/jim-kelly-769a166/profile" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	

				 
				
			</div>

			<div class="col-lg-4 member_card mt-10 mb-10 mt-lg-0 pl-50 pr-50">
				<div class="img-logo d-flex">
	                <a href="docs/NEW%20ACQUISITION%20IN%20CANADA%20FOR%20NCA%20ENTREPRENEUR%20MARTINE%20LEGAULT%20FINAL.pdf" class="img_section w-50">
                        <div class="img_div">
	                	   <img class="w-100" src="img/searcher-martine-legault.png">
	                	</div>   
	                </a>
	                <a href="https://covepro.ca/" target="_self" class="logo_section w-50 bg-white text-center d-flex align-items-center c-logo-border2">
	                	<div class="logo_div">
	                	    <img style="padding-left:20px" class="w-80" src="img/protek.png">
	                	</div>    
	                </a>	
	            </div>   

                <div class="member_content bg-primary py-4 d-flex align-items-center justify-content-center px-4">
                	<div class="text-white">
                    	<h5 class="text-center text-white font__size-22 line__height-30 font__family-efb">
						  Martine Legault
                    	</h5>
	        			<p class="text-center font__size-16 font__family-open-sans line__height-34">
						CEO of Contrôles Véhiculaires Protek <br>
	        			</p>
	        			<div class="icon_div mt-3 text-center">
							<a href="https://www.linkedin.com/in/martine-legault-a74b6429/" target="_self" class="social_icon social_icon_white d-inline-flex border-radius-5 p-1 font__family-avenir">
								<i class="fab fa-linkedin-in font__size-18"></i>
							</a>
						</div>
	        				
				    </div>	
                </div>	

				 
				
			</div>




























			

		</div>	
	</div>	
</section>

<!-- <section class="apply_section bg-white pb-100 ">
	<div class="container">
		<div class="">
			<div class="nca_talent_col">
				<div class="row no-gutters">
                    
                    <div class="col-md-3">
						<div class="talent_img d-flex justify-content-center align-items-center h-100 px-4">
							<img src="img/dummy_img.png">
						</div>
					</div>

					<div class="col-md-9 d-flex align-items-center">
						<div class="program_content bg-primary h-100 w-100 px-5">
							<div class="row h-100 h-sm-auto">
								<div class="col-md-10 talent_top_heading text-white d-flex align-items-center flex-wrap h-100">
                                    <div class="information_div">
										<div class="title_sec mb-3">
											<h4 class="text-white">
												Title
											</h4>
											<p>
												2021-2023
											</p>
										</div>	

										<p>
											Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor Lorem ipsum dolor sit 
										</p>
									</div>	
								</div>
								<div class="col-md-2 d-flex align-items-center logo_social_div">
									<div class="main_content_div">
										<div class="icon_div mb-5 mb-sm-3">
											<a href="#" target="_self" class="social_icon social_icon_white d-inline-flex align-items-center justify-content-center border-radius-5 p-1 font__family-avenir">
												<i class="fab fa-linkedin-in font__size-18"></i>
											</a>
										</div>	
										<div class="logo_div">
											<p class="text-white">
												Logo
											</p>
										</div>	
									</div>	
								</div>	
						    </div>
                        </div>
					</div>
				</div>
			</div>
		</div>
	</div>
</section> -->

</body>
<?php include 'inc/footer.php'; ?>